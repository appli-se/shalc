...
function pythonType(halType) {
    ...
    switch ((base || '').toLowerCase()) {
        ...
        case 'val':
            return 'float';
        case 'array':
            return `list[${pythonType(halType.elementType)}]`;
        case 'record':
        case 'row':
...
function genFunction(fn) {
    ...
    const retType = pythonType(fn.returnType);
    ...
    lines.push(retLine);
    return `def ${fn.name}(${params}) -> ${retType}:\n${indent(lines.join('\n'))}`;
}
...
function genBasicBlocks(block, paramNames) {
    ...
    for (let i = 0; i < arms.length; i++) {
        ...
        lines.push(`${kw} pc == ${arms[i].pc}:`);
        lines.push(indent(arms[i].body));
    }
    lines.push('else:');
